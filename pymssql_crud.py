...
class pymssql_crud(object):
    ...
    def getQuerySelectDataset(self, records=-1):
        ...
        if self.query_text.upper().find("SELECT") != -1 and self.query_text.upper().find("SELECT") < 10:
            if self.debug == True:
            ...
            if self.debug == True: 
                ...
                print("SELECT query not valid! Try other methods, like INSERT, UPDATE, DELETE, EXEC")
            return [].append("NO_DATA")

    # INSERT QUERY
    def insertQueryData(self):
        # CHECK IF QUERY is a SELECT
        if self.query_text.upper().find("INSERT") != -1 and self.query_text.upper().find("INSERT") < 10:
            if self.debug == True:
                print(self.query_text.upper().find("INSERT"))
            try:
                # CREATING NEW CURSOR
                cursor = self.conn.cursor()
                if self.debug == True: 
            ...
            if self.debug == True: 
                print(self.query_text.find("INSERT"))
                print("INSERT query not valid! Try other methods, like SELECT, UPDATE, DELETE, EXEC")
            return -1

        return

    # UPDATE QUERY: TO IMPLEMENT
    def updateQueryData(self):
        return

    # DELETE QUERY: TO IMPLEMENT
    def deleteQueryData(self):
        return

    # TRUNCATE QUERY: TO IMPLEMENT
    def truncateQueryData(self):
        return
